// client/src/pages/ResponseViewerPage.jsx
import { useState, useEffect } from 'react';
import { useParams, Link } from 'react-router-dom';
import axios from 'axios';
import { motion, AnimatePresence } from 'framer-motion';
// 1. Import AnalyticsDashboard (Keep existing)
import AnalyticsDashboard from '../components/results/AnalyticsDashboard'; 
// 2. Import the new ExportModal
import ExportModal from '../components/results/ExportModal';

const ResponseViewerPage = () => {
  const { formId } = useParams();
  const [responses, setResponses] = useState([]);
  const [form, setForm] = useState(null);
  const [loading, setLoading] = useState(true);
  
  // Tab switching state
  const [viewMode, setViewMode] = useState('list'); // 'list' or 'analytics'
  
  // 3. New state for Export Modal
  const [isExportModalOpen, setIsExportModalOpen] = useState(false);

  useEffect(() => {
    const fetchData = async () => {
        try {
            const formRes = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/forms/${formId}`);
            const responsesRes = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/responses/${formId}`);
            
            setForm(formRes.data);
            setResponses(responsesRes.data);
        } catch (error) {
            console.error("Failed to fetch data", error);
        } finally {
            setLoading(false);
        }
    };
    fetchData();
  }, [formId]);

  if (loading) return <p className="text-center text-gray-500 p-8">Loading...</p>;

  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-8">
      <Link to="/responses" className="text-blue-600 hover:underline mb-6 block">&larr; Back to All Forms</Link>
      
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-8 gap-4">
        <div>
            <h1 className="text-3xl md:text-4xl font-bold mb-2 text-gray-900">Responses for "{form?.title}"</h1>
            <p className="text-gray-600">{responses.length} total submission(s)</p>
        </div>

        <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
            {/* 4. Export Button */}
            <button
                onClick={() => setIsExportModalOpen(true)}
                className="px-4 py-2 rounded-md bg-green-100 text-green-700 font-semibold hover:bg-green-200 transition-colors shadow-sm flex items-center justify-center gap-2"
            >
                <span>Export Data</span>
                <span>📥</span>
            </button>

            {/* Tab Switcher */}
            <div className="bg-gray-200 p-1 rounded-lg inline-flex self-start sm:self-auto">
                <button
                    onClick={() => setViewMode('list')}
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${viewMode === 'list' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
                >
                    Individual
                </button>
                <button
                    onClick={() => setViewMode('analytics')}
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${viewMode === 'analytics' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
                >
                    Analytics 📊
                </button>
            </div>
        </div>
      </div>

      {/* 5. Render Export Modal */}
      <AnimatePresence>
        {isExportModalOpen && (
            <ExportModal 
                isOpen={isExportModalOpen} 
                onClose={() => setIsExportModalOpen(false)} 
                form={form} 
                responses={responses} 
            />
        )}
      </AnimatePresence>

      {/* Conditional Rendering for List/Analytics */}
      {viewMode === 'analytics' ? (
        <AnalyticsDashboard form={form} responses={responses} />
      ) : (
        <div className="space-y-6">
          {responses.length > 0 ? (
             responses.map((response) => (
                <div key={response._id} className="bg-white p-6 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center transition-shadow hover:shadow-xl border border-gray-200 shadow-sm border-t-4 border-t-blue-400 gap-4">
                  <div>
                    <p className="font-semibold text-gray-800 text-lg">{response.userEmail}</p>
                    <p className="text-sm text-gray-500 mt-1">
                      Score: <span className={response.score >= (response.totalMarks/2) ? "text-green-600 font-bold" : "text-red-500 font-bold"}>{response.score}</span> / {response.totalMarks}
                    </p>
                    <p className="text-xs text-gray-400 mt-1">
                        Submitted: {new Date(response.submittedAt).toLocaleDateString()}
                    </p>
                  </div>
                  <Link 
                    to={`/results/${response._id}`} 
                    className="bg-sky-500 hover:bg-sky-600 text-white font-bold py-2 px-5 rounded-lg transition-transform hover:scale-105 shadow-md w-full sm:w-auto text-center"
                  >
                    View Result
                  </Link>
                </div>
             ))
          ) : (
            <div className="text-center py-16 bg-white rounded-lg border border-gray-200 shadow-sm">
                <h3 className="text-2xl font-semibold text-gray-800">No responses yet.</h3>
                <p className="text-gray-500 mt-2">Share your form to start collecting data!</p>
            </div>
          )}
        </div>
      )}
    </motion.div>
  );
};

export default ResponseViewerPage;