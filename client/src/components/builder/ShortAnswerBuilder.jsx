...
const ShortAnswerBuilder = ({ onSave, onCancel, initialData = null }) => {
  ...
  useEffect(() => {
    ...
    } else { setText('Short Answer Question'); }
  }, [initialData]);

  const handleSave = () => {
    onSave({ _id: initialData?._id, type: 'ShortAnswer', content: { question: text } });
  };
  ...
  return (
    <div className="p-8 shadow-2xl animate-fadeIn bg-slate-900 text-white rounded-xl border border-pink-500/20 relative">
      ...
      <h3 className="text-xl font-bold mb-6 pb-4 border-b border-white/10 text-white tracking-tight">Edit Short Answer</h3>
