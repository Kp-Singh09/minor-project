...
const HeadingBuilder = ({ onSave, onCancel, initialData = null, theme }) => {
  ...
  return (
    <div className={`p-6 rounded-lg shadow-md animate-fadeIn border ${currentTheme.cardBg} ${isDark ? 'border-gray-600' : 'border-gray-200'}`}>
      <h3 className={`text-xl font-bold mb-4 pb-4 border-b ${currentTheme.text} ${isDark ? 'border-gray-600' : 'border-gray-100'}`}>Edit Heading</h3>
      
      <label className={`block font-semibold mb-2 ${currentTheme.text}`}>Heading Text</label>
      <input
        type="text"
